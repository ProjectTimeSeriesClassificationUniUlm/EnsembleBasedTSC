...
from LoadData import (
    DATASETS_RESNET,
    get_all_datasets_test_train_np_arrays,
)

path_to_datasets = "../../../datasets"

datasets_test_train_data = get_all_datasets_test_train_np_arrays(path_to_datasets)
datasets_test_train_data = {
    k: v for k, v in datasets_test_train_data.items() if k in DATASETS_RESNET
}
...
if __name__ == "__main__":
    for optimizer, optimizer_name in zip(
        [keras.optimizers.Adam, keras.optimizers.SGD], ["adam", "sgd"]
    ):
        for learning_rate in learning_rates:
            train(
                result_csv_path=f"./opt_{optimizer_name}_lr_{str(learning_rate)}.csv",
