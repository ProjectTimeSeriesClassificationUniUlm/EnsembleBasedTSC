...
def preprocess_datasets(datasets, augmentation=False, batch_size=10):
    for ds_name, ds_data in datasets.items():
        ...
        ds_data["train_data"] = preprocess_data(ds_data["train_data"][0],\
...
def preprocess_data(x_data, y_data, augmentation=False, batch_size=10):
    data_augmentation = keras.Sequential([AdditiveWhiteGaussianNoise(),])
    dataset = tf.data.Dataset.from_tensor_slices((x_data, y_data))
    if augmentation:
        #dataset = dataset
        dataset = dataset.map(lambda x, y: (data_augmentation(x), y)).batch(batch_size)\
...
class AdditiveWhiteGaussianNoise(Layer):
    ...
    def __init__(self, min_noise_stddev: int = 0.05, max_noise_stddev: int = 0.4, name="AdditiveWhiteGaussianNoise", **kwargs):
    ...
    def call(self, inputs):
        ...
        outputs = tf.py_function(add_additive_white_gaussian_noise, [inputs, noise_stddev], np.float32)
