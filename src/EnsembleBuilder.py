...
class EnsembleBuilder():
    def __init__(self, dataset_names=[CurrentDatasets.swedish_leaf.value, CurrentDatasets.fifty_words.value],
    ...
    def run_ensembles(self, augmentation=False):
        ...
        for dataset_name in self.dataset_names:
            ...
            for ensemble_name, model_names in self.ensembles.items():
                for row in self._run_ensemble(ensemble_name, model_names, dataset_name, augmentation):
                    result.loc[len(result)] = row
            i = i + 1
        return result

    
    def _run_ensemble(self, ensemble_name, model_names=['Encoder'], evaluation_dataset=CurrentDatasets.swedish_leaf.value, augmentation=False):
        """
        ...
        x_test, y_test = get_all_datasets_test_train_np_arrays(self.datasets_path, [evaluation_dataset])[evaluation_dataset][
            'test_data']
        if augmentation:
            x_test = np.array(list(map(add_additive_white_gaussian_noise, x_test)))

        predicted_classes = self._get_ensemble_predictions(x_test, evaluation_dataset, model_names)

        dataset_names = [evaluation_dataset for _ in self.ensemble_methods]
        display_names = [f"{ensemble_name}-{method_name}" for method_name in self.ensemble_methods]
    ...
    def _get_ensemble_predictions(self, x, evaluation_dataset, model_names, check_identical=False):
        """
    ...
    def _load_models(self, dataset_name=CurrentDatasets.swedish_leaf.value, model_names=['Encoder']):
        if self.models_path is None:
            self.models_path = f'../models/'
        curr_models_path =  os.path.join(self.models_path, dataset_name)
        models_to_load = list(
            filter(lambda model_name: remove_suffix(model_name,'.h5') in model_names, os.listdir(curr_models_path)))
        models = list(map(lambda filename: keras.models.load_model(curr_models_path + "/" + filename), models_to_load))
        return models
